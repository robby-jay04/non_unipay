...
class StudentController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        ...
        // ✅ Update name and/or email on the users table if provided
        if (isset($validated['email']) || isset($validated['name'])) {
            ...
            $request->user()->update($userUpdates);
            unset($validated['email'], $validated['name']);
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            // Prevent accidental deletion if related records exist
            ...
            // Delete the associated user (cascades to student if FK set up)
            if ($student->user) {
                $student->user->delete();
            } else {
                $student->delete();
